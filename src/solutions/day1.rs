...
pub fn task1(path: &Path) -> u32 {
    ...
    let amounts = blocks.iter().map(|b| b.lines().map(|l| l.parse::<u32>().unwrap()).sum()).collect::<Vec<u32>>();
...
pub fn task2(path: &Path) -> u32 {
    let blocks = get_input_by_block(path);
    let mut amounts = blocks.iter().map(|b| b.lines().map(|l| l.parse::<u32>().unwrap()).sum()).collect::<Vec<u32>>();
    amounts.sort();
    let top_three = amounts[amounts.len()-1] + amounts[amounts.len()-2] + amounts[amounts.len()-3];
    top_three
}
